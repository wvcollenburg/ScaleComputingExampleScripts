...
Script to demonstrate creating a clone from a replica and exporting it for backup purposes
...
$smbdomain = "company.local"                        # The domain for the SMB user. usually your windows domain
$smbuser = "Administrator"                          # Username that has write and change rights on the SMB share
$smbpassword = "S3cr3t5!"                           # Password for given user
$smburl = "fileserver.company.local"                # IP Address or FQDN for SMB share
$smbpath = "/exports"                               # SMB share path. Needs to start with a / and needs to end without a /

$BackupTag = "MakeReplicaBackup"                    # This tag needs to be added to VMs that you would like to be exported by this script
...
ForEach ($vm in $readInfo) {
    If ($vm.tags -like $searchTag) {
        ...
        $cloneURL = "https://$node/rest/v1/VirDomain/$vmid/clone"
        $result = Invoke-RestMethod -Method 'Post' -Uri $cloneURL -WebSession $mywebsession @restOpts -Body $Body
        ...
        $exportURL = "https://$node/rest/v1/VirDomain/$clonedVM/export"
        ...
        $deleteURL = "https://$node/rest/v1/VirDomain/$clonedVM"
        $deleteresult = Invoke-RestMethod -Method 'Delete' -Uri $deleteURL -WebSession $mywebsession @restOpts
        Wait-ScaleTask -TaskTag $($deleteresult.taskTag)
